...
public class LLCam implements Subsystem {
    ...
    public LLCam(Limelight3A limelight, int pipelineIndex) {
    ...
    private void updateTarget() {
        ...
        if (result != null && result.isValid()) {
            ...
            target.distance = ShooterConstants.TARGET_Y / tan;
            return;
        }
    ...
    public void updateRobotPose(){
        if(target.hasTarget && target.distance < CameraConstants.poseUpdateDistance && Robot.getMagnitude() < CameraConstants.poseUpdateVelocity && time-lastRobotPoseUpdateTimeMS > CameraConstants.poseUpdateIntervalMS) {
    ...
    private void publishTelem() {
        PerTelem.addData("LL Has Target", target.hasTarget);
        if (!target.hasTarget) return;

        PerTelem.addData("LL Tag ID", target.id);
        PerTelem.addData("LL tX", target.tX);
        PerTelem.addData("LL tY", target.tY);
        PerTelem.addData("LL Target Distance", target.distance);
    }
